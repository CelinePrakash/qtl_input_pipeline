...
def parse_arguments():
	...
	parser.add_argument('--numberofparents', '-n', help='Number of samples that are parents', type=int)
...
def too_many_miscalls_check(inputmatrix,maxrecombinationindividuals):
	...
	##note for the F1 cross heatmap, the last position of the matrix is not the position genotype
...
def determine_recombination(inputmatrix,header,minmarkersforrecombinationregion,numofparents):
	...
	#F1 requires input number of parents and should be excluded from the recombination determination
	exclude_samples=[]
	samplesrecombination=[]
	noindividuals=len(inputmatrix[0])-(3+numofparents) #1. chr_position, 2. GP1, 3. GP2, (+numberofparents) & no position genotype
	chrposition=[]
	QTLtable={}
	for row in range(len(inputmatrix)):
		chrposition.append(inputmatrix[row][0])
	for ind in range(noindividuals):
		individual=ind+(3+numofparents)
		individualvector=[]
		...
		for row in range(len(inputmatrix)):
			...
			if abs(float(inputmatrix[row][individual]))==float(1):
				...
				subsetindividualvector.append(inputmatrix[row][individual])
				originalindex.append(row)
			elif float(inputmatrix[row][individual])==0.5:
				#missing
		...
		for j in range(len(recombinationindex)):
			if mode(individualvector[start:(recombinationindex[j]+1)])=='1':
				value.extend(['AA']*(recombinationindex[j]-start))
			elif mode(individualvector[start:(recombinationindex[j]+1)])=='0':
				value.extend(['AB']*(recombinationindex[j]-start))
			elif mode(individualvector[start:(recombinationindex[j]+1)])=='-1':
				value.extend(['BB']*(recombinationindex[j]-start))
			else:
				for individualpos in range(start,(recombinationindex[j])):
					if individualvector[individualpos]=='1':
						value.append('AA')
					if individualvector[individualpos]=='0':
						value.append('AB')
					if individualvector[individualpos]=='-1':
						value.append('BB')
...
if __name__ == '__main__':
	...
	for line in inf:
		if line.startswith("#CHROM"):
			...
			if firstline:
				...
				firstline=False
			#check if there are only 2 alleles for the marker based on the ALT column (col 5) not containing ','
			if ',' not in linesplit[4]:
				...
				if fractionsamplesdepthfilter>args.fractionindividuals:
					...
					if linesplit[0] != currentchr:
							...
							if len(outlistlist)>0:
								...
								if len(outlistlist)>0:
									...
									if len(outlistlist)>0:
										...
										samplesrecombination,exclude_samples,QTLtable=determine_recombination(outlistlist,headerlist,args.minmarkersforrecombinationregion,args.numberofparents)
						...
						outlistlist=[]
					##check grandparent genotypes
					grandparent1genotype=linesplit[9].split(":")[GT_index].split("/")
					grandparent2genotype=linesplit[10].split(":")[GT_index].split("/")
					grandparent1allele=grandparent1genotype[0]
					grandparent2allele=grandparent2genotype[1]
					#specify grandparent1's genotype as 1 and grandparent2's genotype as -1
					outlist.append('1')
					outlist.append('-1')
					##check parent genotypes and specify in matrix
					hetero_genotype=str(min(int(grandparent1allele),int(grandparent2allele)))+"/"+str(max(int(grandparent1allele),int(grandparent2allele)))
					for parentindex in range(args.numberofparents):
						parentgenotype=linesplit[11+parentindex].split(":")[GT_index].split("/")
						if grandparent1allele in parentgenotype and grandparent2allele in parentgenotype:
							outlist.append('0')
						elif set(parentgenotype)==set(grandparent1genotype):
							outlist.append('1')
						elif set(parentgenotype)==set(grandparent2genotype):
							outlist.append('-1')
						else:
							outlist.append('-0.5')
					...
					offspringvector=[]
					skip=False
					for i in range(11+args.numberofparents,len(linesplit)):
						if len(linesplit[i])==0 or linesplit[i].split(":")[GT_index]=="./.":
							offspringvector.append(0.5)
							missing_count+=1
						elif linesplit[i].split(":")[GT_index]==hetero_genotype:
							het_count+=1
							offspringvector.append(0)
						elif linesplit[i].split(":")[GT_index]==linesplit[9].split(":")[GT_index]:
							#grandparent1 genotype
							hom1_count+=1
							offspringvector.append(1)
						elif linesplit[i].split(":")[GT_index]==linesplit[10].split(":")[GT_index]:
							#grandparent2 genotype
							hom2_count+=1
							offspringvector.append(-1)
						else:
							##unexpected genotype
							...
							#skip=True
							offspringvector.append(-0.5)
					if (float(unexpected_count+missing_count)/(het_count+hom1_count+hom2_count+unexpected_count+missing_count))>args.maxdiffmissing:
						#more than 25% of the sites have either a genotype that differ from the parents or missing information
						print("Too many genotypes differ from grandparents, parents or missing information. Unexpected: %s, Missing:%s"%(unexpected_count,missing_count))
						skip=True
					if (het_count+hom1_count+hom2_count)>0:
						if float(min(het_count,hom1_count,hom2_count))/(het_count+hom1_count+hom2_count) < args.mendelianfraction:
							##does not meet mendalian fraction expectation
							print("Does not meet mendalian fraction expectation. Heterozygous: %s, Homozygous A:%s, Homozygous B:%s"%(het_count,hom1_count,hom2_count))
							skip=True
					if not skip:
	...
	if len(finaloutlistlist)>0:
		...
		for chr in chrlist:
			...
			cbar.set_ticklabels(['Homozygous B','Unexpected','Heterozygous','No Call','Homozygous A'])
